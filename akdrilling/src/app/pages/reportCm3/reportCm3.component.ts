...
@Component({
  selector: "app-reportCm3",
  templateUrl: "reportCm3.component.html",
  styleUrls: ["reportCm3.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm3Component implements OnInit {
  periodos!: Periodos[];
  usuario!: any;
  loadingPage: boolean;
  PERIODO_REQ: boolean;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM3(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
