...
@Component({
  selector: "app-reportCm2",
  templateUrl: "reportCm2.component.html",
  styleUrls: ["reportCm2.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm2Component implements OnInit {
  periodos!: Periodos[];
  proyectos!: Proyectos[];
  proyectlist: Array<[]>;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM2(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
