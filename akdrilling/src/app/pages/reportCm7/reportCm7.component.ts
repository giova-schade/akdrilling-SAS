...
@Component({
  selector: "app-reportCm7",
  templateUrl: "reportCm7.component.html",
  styleUrls: ["reportCm7.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm7Component implements OnInit {
  periodos!: Periodos[];
  usuario!: any;
  loadingPage: boolean;
  PERIODO_REQ: boolean;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM7(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
