...
@Component({
  selector: "app-reportCm5",
  templateUrl: "reportCm5.component.html",
  styleUrls: ["reportCm5.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm5Component implements OnInit {
  periodos!: Periodos[];
  proyectos!: Proyectos[];
  proyectlist: Array<[]>;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM5(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
