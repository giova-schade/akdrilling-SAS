...
@Component({
  selector: "app-reportCm6",
  templateUrl: "reportCm6.component.html",
  styleUrls: ["reportCm6.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm6Component implements OnInit {
  periodos!: Periodos[];
  usuario!: any;
  loadingPage: boolean;
  PERIODO_REQ: boolean;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM6(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
