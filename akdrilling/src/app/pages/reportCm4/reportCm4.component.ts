...
@Component({
  selector: "app-reportCm4",
  templateUrl: "reportCm4.component.html",
  styleUrls: ["reportCm4.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm4Component implements OnInit {
  periodos!: Periodos[];
  usuario!: any;
  loadingPage: boolean;
  PERIODO_REQ: boolean;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM4(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
