...
@Component({
  selector: "app-reportCm1",
  templateUrl: "reportCm1.component.html",
  styleUrls: ["reportCm1.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm1Component implements OnInit {
  periodos!: Periodos[];
  usuario!: any;
  loadingPage: boolean;
  PERIODO_REQ: boolean;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM1(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
