...
@Component({
  selector: "app-reportCm8",
  templateUrl: "reportCm8.component.html",
  styleUrls: ["reportCm8.component.scss"],
  providers: [NotificationsComponent, ConfirmationService]
})
export class ReportCm8Component implements OnInit {
  periodos!: Periodos[];
  usuario!: any;
  loadingPage: boolean;
  PERIODO_REQ: boolean;
  ...
  datosReporteCM() {
    ...
    this.master.apiPostPeriodReportCM8(this.reportcm).subscribe({
      next: (result: any) => {
        if (result.status == "ok") {
